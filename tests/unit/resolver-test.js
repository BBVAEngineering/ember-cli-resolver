/* eslint-disable ember/no-new-mixins, ember/avoid-leaking-state-in-ember-objects */
...
module('Unit | Mixin | resolver', (hooks) => {
	hooks.beforeEach(() => {
		const BaseApplication = Application.extend({
			name: 'application',
	...
	test('it loads components modules from loader.js when required twice', (assert) => {
		const object = Component.extend();
	...
	test('it reopens module with properties when resolved', (assert) => {
		...
		assert.ok(resolved.PrototypeMixin.mixins.length === 2, 'reopen is included');
		assert.equal(resolved.PrototypeMixin.mixins[0].properties, reopen.reopen[0], 'reopen matches with first defined');
		assert.equal(resolved.PrototypeMixin.mixins[1].properties, reopen.reopen[1], 'reopen matches with second defined');
	});
	...
	test('it reopens module with mixin when resolved', (assert) => {
		...
		const reopen = {
			for: 'dummy/object',
			reopen: [Mixin.create({ foo: 'bar' }, { bar: 'foo' })]
		};
		...
		assert.ok(resolved.PrototypeMixin.mixins[0], 'reopen is included');
		assert.equal(resolved.PrototypeMixin.mixins[0], reopen.reopen[0], 'reopen matches with defined');
	});
	...
	test('it reopens class with mixin when resolved', (assert) => {
		const moduleName = 'dummy/object';
		const reopenModule = 'dummy/reopens/object';
		const object = EmberObject.extend();
		const reopen = {
			for: 'dummy/object',
			reopenClass: [Mixin.create({ foo: 'bar' }, { bar: 'foo' })]
		};
